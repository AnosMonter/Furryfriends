...
class Database
{
    ...
    function tim_kiem_sp($keyword, $min_price, $max_price, $order)
    {
        $sql = "SELECT * FROM Products WHERE Name LIKE :keyword AND Price BETWEEN :min_p AND :max_p AND Status = 1 ORDER BY Price $order";
        $stmt = $this->pdo->prepare($sql);
        $keyw = '%'.$keyword.'%';
